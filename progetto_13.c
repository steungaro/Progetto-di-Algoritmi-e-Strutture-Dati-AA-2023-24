...
typedef struct ordine { //tipologia di dato ordine: contiene il puntatore alla ricetta, l'istante di ordine, la quantità, l'ordine successivo, il precedente e il peso
    unsigned int t;
    ricetta_t* ricetta;
    unsigned int q;
    unsigned int peso;
...
void elimina_hash_magazzino (magazzino_hash_t* magazzino) { // accetta un magazzino e lo elimina, eliminando anche la lista degli ingredienti e dei lotti al suo interno
    ...
    for (i=0; i < DIM_MAGAZZINO; i++) { //per ogni elemento del magazzino
        while (magazzino->stock_ingredienti[i] != NULL){ // se contiene qualcosa
...
/*void elimina_listascadenze (scadenze_t* listascadenze) { //accetta la lista delle scadenze e la elimina, effettuando le free
    scadenze_t* tempscadenza;
    if (listascadenze) {
...
void elimina_coda (ordine_t *coda) { //accetta una coda e la elimina, effettuando le free
    ordine_t* temp;
    while (coda != NULL) {
        temp = coda;
...
/*scadenze_t *aggiungi_scadenza (scadenze_t* lista, unsigned int sc) { //accetta la lista delle scadenze e aggiunge la scadenza passata, se già non è presente
    scadenze_t *tempscadenza, *scorr, *prec;
    int found;
    tempscadenza = NULL;
    prec = NULL;
    scorr = lista;
        ...
        while (scorr != NULL && !found) {
            if (scorr->s >= sc) {
                ...
                if (tempscadenza) {
                    tempscadenza->s = sc;
                    if (prec != NULL) {
                        tempscadenza->next = prec->next;
                        prec->next = tempscadenza;
                    } else {
...
ordine_t* aggiungi_ordine_in_coda(ordine_t* coda, ordine_t **last, ricetta_t* ricetta, unsigned int q, unsigned int t, unsigned int *ret) { // USARE AGGIUNGI ORDINE accetta la coda degli ordini (NULL se vuota), l'ultimo elemento, la ricetta dell'ordine da eseguire, la quantità, il tempo assoluto e restituisce 1 se l'operazione è andata a buon fine e -1 altrimenti
...
ordine_t* aggiungi_ordine(ordine_t* coda, ordine_t **last, ricettario_hash_t* ricettario, char* nome, unsigned int q, unsigned int t, unsigned int *ret) { //accetta una coda degli ordini (NULL se vuota), il ricettario, il nome della ricetta, la quantità e il tempo assoluto e restituisce 1 se l'ordine è andato a buon fine, NULL se la ricetta non esiste. inoltre aggiunge pending alla ricetta in questione
    ...
    if (ricetta) {
        ricetta->pending ++;
        return aggiungi_ordine_in_coda(coda, last, ricetta, q, t, ret);
    } else {
        *ret = -1;
        return coda;
    }
...
/*
ordine_t* aggiungi_ordine_in_lista1(ordine_t* lista, ordine_t* ordine) { //accetta la lista di prodotti pronti da spedire, il ricettario, l'ordine (mantiene l'entità ordine, non fare free ma toglierne riferimento dalla coda ordini) e aggiunge l'ordine passato alla coda degli ordini completati ordinata per tempo
    ordine_t* temp, *prec;
    int stop;
...
void togli_ingrediente(magazzino_hash_t* magazzino, char nome[], int h, unsigned long long qt) {
    int stop;
    ...
    for (ingrediente_temp = magazzino->stock_ingredienti[h]; ingrediente_temp != NULL && !stop; ingrediente_temp = ingrediente_temp->next) { //percorro tutti gli ingredienti finché non trovo quello col nome giusto oppure finché non li ho esplorati tutti
        if (!strcmp(ingrediente_temp->nome, nome)) { // se i nomi combaciano
            stop = 1;
            if (ingrediente_temp->totale >= qt) {
...
ordine_t* cucina(magazzino_hash_t* magazzino, ordine_t* codaordini, ordine_t** codaprodotti, ordine_t **last, int tipo) { //se di tipo 1 controlla solo l'ultimo, se di tipo 0 controlla tutti gli ordini e vede se sono cucinabili, in tal caso li cucina
    int cucinabile;
        ...
        if (cucinabile) {
            ...
            if (temp_ordine == NULL) {
                *last = prodotto->prev;
                ...
                temp_ordine->prev = prodotto->prev;
            }
            if (prodotto->prev != NULL) {
                prodotto->prev->next = temp_ordine;
            } else {
                codaordini = temp_ordine;
                if (temp_ordine) {
                    temp_ordine->prev = NULL;
                } else {
                    *last = NULL;
                }
            }
...
ordine_t* consegne (unsigned int peso, ordine_t* coda) { //controlla tutti i prodotti disponibili e fa il controllo per il peso del furgoncino, poi li spedisce in ordine di peso
...
int main () {
    ...
    char nl;
    ricettario_hash_t *ricettario;
    ordine_t *coda_ordini, *coda_prodotti, *last;
    magazzino_hash_t *magazzino;
    ingrediente_t *lista_ingredienti;
    ricetta_t *tempricetta;

    //if(freopen("open7.txt", "r", stdin));
...
//                    listascadenze = aggiungi_scadenza(listascadenze, sc);
            ...
            coda_ordini = cucina(magazzino, coda_ordini, &coda_prodotti, &last, 0);
            ...
            if(ret == 1) {
                printf("accettato\n");
                scadenze(magazzino, t);
                coda_ordini = cucina(magazzino, coda_ordini, &coda_prodotti, &last, 1);
            } else {
                printf("rifiutato\n");
            }
...
//    elimina_listascadenze (listascadenze);
    return 0;    
}