...
ordine_t* aggiungi_ordine_in_coda(ordine_t* coda, ricetta_t* ricetta, int q, int t) { // USARE AGGIUNGI ORDINE accetta la coda degli ordini (NULL se vuota), la ricetta dell'ordine da eseguire, la quantità, il tempo assoluto e restituisce 1 se l'operazione è andata a buon fine e -1 altrimenti
...
ordine_t* cucina(magazzino_hash_t* magazzino, ordine_t* codaordini, ordine_t** codaprodotti) { //dopo l'aggiunta di un ordine o dopo il rifornimento va avviata: controlla gli elementi della coda ordini e prepara quelli i cui ingredienti sono disponibili nel magazzino, restituisce la nuova coda ordini e modifica la coda prodotti passata
    ...
    while (temp_ordine != NULL) {
        ...
        for(temp_ingrediente = temp_ordine->ricetta->lista_ingredienti; temp_ingrediente != NULL; temp_ingrediente = temp_ingrediente->next) {
            cucinabile *= abbastanza_ingrediente(magazzino, temp_ingrediente->nome, temp_ingrediente->q * temp_ordine->q);
        }
        if (cucinabile) {
            for(temp_ingrediente = temp_ordine->ricetta->lista_ingredienti; temp_ingrediente != NULL; temp_ingrediente = temp_ingrediente->next) {
...
ordine_t* consegne (int peso, ordine_t* coda) { //controlla tutti i prodotti disponibili e fa il controllo per il peso del furgoncino, quando l'ordine viene spedito va fatta la free dell'entità ordine
    ordine_t  *next_prodotto, *consegna, *temp_consegna, *prec_consegna;
        ...
        while (coda != NULL && stop) { //scorro tutti i prodotti fino a quando non arrivo a NULL o finché non ho trovato il primo che non ci sta sul camioncino (scorro in ordine temporale)
            if (coda->peso <= residuo) { //se il peso del mio prodotto è minore= di quanto ancora posso caricare
                ...
                if (consegna == NULL) { //aggiunta del prodotto nella nuova coda consegne, se la lista è vuota prende il prodotto e basta
                    ...
                    while (temp_consegna != NULL && (temp_consegna->peso > coda->peso || (!(temp_consegna->peso == coda->peso)||(temp_consegna->t < coda->t)))) { //scorro finché la temp_consegna non punta a qualcosa di peso <= al mio ordine
                    ...
                    if (temp_consegna == NULL) {
                        ...
                        coda->next = NULL;
                        coda = next_prodotto;
                    } else if (temp_consegna->peso < coda->peso) { //se la consegna è più leggera
...
int main () {
    ...
    while(scanf(" %s", input) != -1) {
        ...
        if (!strcmp(input, "aggiungi_ricetta")) { 
            ...
            coda_ordini = aggiungi_ordine(coda_ordini, ricettario, nome, qt, t);
            if(coda_ordini) {
                printf("accettato\n");
                coda_ordini = cucina(magazzino, coda_ordini, &coda_prodotti);
            } else {
